...
Version:2.1
Description:
    - 此版本删除多余的可供学习参考使用的类或函数
    — 使用jkeypoints.py的Describer类实现网络资源获取
"""
import os
import re
import requests
...
class Fetch:
    ...
    def __init__(self, task=('', '')):
        self.__keyword, self.__savepath = task
        desc = Describer(self.__keyword).info
        self.__target = desc.get('image')  # 目标资源的URL
        self.__title = desc.get('title')  # 下到本地的文件名(不含扩展名)
        self.__retrieve()

    def __retrieve(self):  # 等同于recover()
        """使用requests.get()把目标url资源下载到指定位置的方法"""
        dst = self.__savepath + os.sep + self.__title + os.path.splitext(self.__target)[1]
        telefile = requests.get(self.__target)
    ...
    def __del__(self):
        """"""
        print('{}任务完成'.format(self.__keyword).center(20, '*'))
...
class Reunion:
    ...
    def __init__(self, root_dir):
        ...
        if self.todo:
            ...
            for t, d in self.todo:
                print('{:\t<8} =>\t{}'.format(t, d))
            print('\n任务个数共计:{}'.format(self.todo.__len__()))
    ...
    def unit(self):
        while self.todo:
            ...
            print('{:0>3} 正在将{:<11} 的资源下载到{}'.format(self.i, task[0], task[1]))
            Fetch(task)
